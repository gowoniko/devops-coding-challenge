resource "aws_route_table" "route_table" {
  vpc_id = var.vpc_id
  tags = {
    Name        = "${var.service}-devops-challenge-${var.env}-routetable-${var.is_public ? "pub" : "priv"}"
    Environment = "${var.env}"
  }
}
...
resource "aws_route" "ipv4_route" {
  ...
  gateway_id                = each.value.route[1] == "internet_gw" ? try(lookup({ for gateway in local.gateways : "${gateway.id}" => gateway }, "${each.value.route[1]}").value, null) : null
